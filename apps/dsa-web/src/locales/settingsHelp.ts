...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.EXTERNAL_TOOL_ENABLED': {
    title: 'ExternalTool 研究扩展',
    summary: '控制是否在 DSA 中启用独立的 ExternalTool 量化研究能力。',
    usage: '默认关闭。开启前需在 DSA 使用的同一 Python 环境中安装已验证的 ExternalTool 版本。',
    valueNotes: ['该开关不会自动安装或升级私有仓库代码。', '关闭或缺包时，DSA 原有分析、报告与通知不受影响。'],
    impact: ['影响研究台入口和 /api/v1/external-tool 下的扩展 API。'],
    notes: ['DSA 只导入 EXTERNAL_TOOL_ADAPTER_MODULE 指定的固定模块，不扫描任意源码目录，也不启动第二个端口。'],
  },
  'settings.data_source.EXTERNAL_TOOL_DATA_DIR': {
    title: 'ExternalTool 数据目录',
    summary: '指定 ExternalTool 的本地行情、缓存和研究数据库目录。',
    usage: '建议使用相对路径，例如 data/external_tool。',
    valueNotes: ['该值是数据目录，不是 ExternalTool 源码目录。'],
    impact: ['影响本地研究数据的读写位置。'],
    notes: ['GitHub Actions 使用 Runner 临时目录，本机数据不会自动同步到 Actions。'],
  },
  'settings.data_source.EXTERNAL_TOOL_ADAPTER_MODULE': {
    title: '外部工具适配模块',
    summary: '指定已安装外部工具暴露的 Python 适配模块导入路径。',
    usage: '填写点分 Python 模块名；真实模块名只保存在本地配置或受保护的 Actions Secret 中。',
    valueNotes: ['该值不是源码目录，也不会触发任意目录扫描。'],
    impact: ['影响外部工具的加载与合同校验。'],
    notes: ['模块缺失或格式无效时，扩展会降级且不阻断 DSA 原生功能。'],
  },
  'settings.data_source.EXTERNAL_TOOL_AUTOMATION_CONFIG': {
    title: 'ExternalTool 自动化配置',
    summary: '指定每日报告预览或 Actions 批处理使用的版本化 YAML 配置。',
    usage: '仅在报告预览和自动化批处理时设置，内容不得包含邮箱、Token 或 API Key。',
    valueNotes: ['本地研究台状态不会自动写入这份配置。'],
    impact: ['影响自动化股票列表、研究方法和自定义 Markdown。'],
    notes: ['Actions 中应指向私有外部工具仓库检出目录内的配置文件。'],
  },
  'settings.data_source.EXTERNAL_TOOL_CONTRACT_MIN': {
    title: 'ExternalTool 最低合同版本',
    summary: '定义此 DSA fork 接受的最低适配合同版本。',
    usage: '由维护者随集成代码管理，普通使用者保持默认值。',
    valueNotes: ['低于该版本的引擎会被标记为不兼容。'],
    impact: ['影响扩展兼容性检查。'],
    notes: ['升级前应先运行适配合同测试。'],
  },
  'settings.data_source.EXTERNAL_TOOL_CONTRACT_MAX': {
    title: 'ExternalTool 最高合同版本',
    summary: '定义此 DSA fork 接受的最高适配合同版本。',
    usage: '由维护者随集成代码管理，普通使用者保持默认值。',
    valueNotes: ['高于该版本的引擎会被标记为不兼容。'],
    impact: ['影响扩展兼容性检查。'],
    notes: ['不得通过放宽版本号绕过不兼容变更的测试。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.EXTERNAL_TOOL_ENABLED': {
    title: 'ExternalTool Research Extension',
    summary: 'Controls the isolated ExternalTool quantitative research capability in DSA.',
    usage: 'Disabled by default. Install a verified ExternalTool version into the same Python environment before enabling it.',
    valueNotes: ['This switch never installs or upgrades private repository code.', 'DSA analysis, reports, and notifications remain available when disabled or missing.'],
    impact: ['Affects the research workspace and extension APIs under /api/v1/external-tool.'],
    notes: ['DSA imports only the module configured by EXTERNAL_TOOL_ADAPTER_MODULE, scans no arbitrary source directory, and starts no second port.'],
  },
  'settings.data_source.EXTERNAL_TOOL_DATA_DIR': {
    title: 'ExternalTool Data Directory',
    summary: 'Selects the local directory for ExternalTool market data, caches, and research database.',
    usage: 'Prefer a relative path such as data/external_tool.',
    valueNotes: ['This is a data directory, not a ExternalTool source-code directory.'],
    impact: ['Affects where local research data is read and written.'],
    notes: ['GitHub Actions uses temporary runner storage; local data is not synchronized automatically.'],
  },
  'settings.data_source.EXTERNAL_TOOL_ADAPTER_MODULE': {
    title: 'External Tool Adapter Module',
    summary: 'Sets the Python import path exposed by the installed external tool.',
    usage: 'Use a dotted Python module name; keep the real module only in local configuration or a protected Actions secret.',
    valueNotes: ['This is not a source directory and does not enable arbitrary path scanning.'],
    impact: ['Affects external-tool loading and contract validation.'],
    notes: ['A missing or invalid module degrades the extension without blocking native DSA behavior.'],
  },
  'settings.data_source.EXTERNAL_TOOL_AUTOMATION_CONFIG': {
    title: 'ExternalTool Automation Config',
    summary: 'Selects the versioned YAML used for daily report previews or Actions batch runs.',
    usage: 'Set it only for report previews and automation. Never store email credentials, tokens, or API keys in it.',
    valueNotes: ['Local research workspace state is not written to this file automatically.'],
    impact: ['Affects automated symbols, research methods, and custom Markdown.'],
    notes: ['In Actions, point it to the configuration inside the private external tool checkout.'],
  },
  'settings.data_source.EXTERNAL_TOOL_CONTRACT_MIN': {
    title: 'ExternalTool Minimum Contract',
    summary: 'Defines the oldest adapter contract accepted by this DSA fork.',
    usage: 'Maintainers update it with integration code; ordinary users should keep the default.',
    valueNotes: ['Older engines are reported as incompatible.'],
    impact: ['Affects extension compatibility checks.'],
    notes: ['Run adapter contract tests before upgrading.'],
  },
  'settings.data_source.EXTERNAL_TOOL_CONTRACT_MAX': {
    title: 'ExternalTool Maximum Contract',
    summary: 'Defines the newest adapter contract accepted by this DSA fork.',
    usage: 'Maintainers update it with integration code; ordinary users should keep the default.',
    valueNotes: ['Newer engines are reported as incompatible.'],
    impact: ['Affects extension compatibility checks.'],
    notes: ['Do not widen this range to bypass compatibility tests.'],
  },
  'settings.data_source.REALTIME_SOURCE_PRIORITY': {
