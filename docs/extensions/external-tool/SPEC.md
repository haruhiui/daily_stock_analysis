...
研究台页面由 ExternalTool 随 Python 包发布，DSA 只提供通用托管容器。迁移页面通过构建、合同测试和真实浏览器验证后，DSA 内旧研究台页面直接删除，避免双份前端长期漂移。
...
```text
apps/dsa-web/src/features/external-tool/       # 状态与页面清单 API
apps/dsa-web/src/features/hosted-surfaces/     # 通用页面加载器
```
...
挂载点不得包含研究业务状态、表单或结果渲染。导航根据后端状态接口决定是否展示研究台；扩展异常不得破坏主导航。通用加载器只理解 `surface_id`、入口资源、样式资源、主题、语言和导航回调，不得导入 ExternalTool 源码或依赖其前端目录结构。
...
GET  /api/v1/external-tool/watchlists
GET  /api/v1/external-tool/surfaces/{surface_id}
GET  /api/v1/external-tool/surfaces/{surface_id}/assets/{asset_path}
...
- 托管页面资源只能来自适配器固定清单；后端验证文件名、Base64 编码和 SHA-256，不接受任意文件路径。

## 8. 本地研究台 UI

`/research` 路由挂载 ExternalTool 提供的 `research` 托管页面。页面源码、业务组件、表单、结果渲染和样式归 ExternalTool 所有；DSA 只负责同源转发固定资源、动态挂载以及传入当前主题、语言和宿主导航回调。

ExternalTool 的托管页面使用中性目录和协议名称，不包含宿主项目名称。页面在开放 Shadow DOM 中运行，样式与 DSA 隔离；浏览器仍只访问 DSA 同源 API，不启动第二个前端或后端服务。托管页面必须同时支持浅色、深色和移动端布局。

页面顶部先显示 ExternalTool 通过适配合同提供的只读研究自选列表，再显示靠左对齐的研究工具卡。自选列表使用紧凑卡片网格；选择标的只填入本次单标的研究参数，不修改 DSA `STOCK_LIST`、ExternalTool 自选数据库或自动化配置。
...
选择“研究方法”后，页面根据适配层注册表展示方法列表。通用方法使用 Schema 表单及摘要、表格、时间序列渲染器，专用 renderer 必须注册在 ExternalTool 托管页面源码中。新增普通研究方法不得修改 DSA 主导航或路由。

选择工具后在同页下方加载对应工作区。公式画布默认第一项并直接进入脚本模式，不提供表达式模式切换；函数目录由 ExternalTool 按类型提供，图表由托管页面根据 ExternalTool 的 panels/series/kline/overlays 合同渲染。长任务沿用 DSA 后端任务生命周期，但进度和错误 UI 由托管页面呈现；结果默认包含表格、时间序列或图表及可解释字段，不只显示单个数字。
...
DSA 的 `STOCK_LIST` 和本次请求标的是 DSA 原生逐股 LLM 深度分析的真源，也可以作为 ExternalTool 研究方法的一次性输入，但不再被定义为 ExternalTool 全部研究范围的唯一真源。ExternalTool 可以维护独立且更大的研究池，用于确定性选股、筛选和回测；DSA 只通过可选 `watchlists` 适配能力读取结构化列表，不读取其内部数据库表。
...
- 本地研究台由适配层提供托管资源，并通过 DSA API 调用适配层；
